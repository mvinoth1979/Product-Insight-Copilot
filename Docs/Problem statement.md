Build an AI-powered workflow that turns raw customer reviews into actionable product insights and ready-to-use support communication. You will analyze recent user feedback to identify key themes, detect recurring confusion around a specific fee or charge, and convert these insights into a structured weekly product update and a clear customer-facing explanation. Finally, the system should use Model Context Protocol (MCP) with an approval step to log these outputs into internal tools and prepare an email draft. This simulates a real-world setup where product teams monitor user sentiment and support teams proactively address common user confusion.
Milestone 2
Problem Statement
...
Your system must:
scarpe user reviews for an APP from Google Play Store
Remove any duplicate reviews, old reviews, and irrelevant reviews
Create CSV file with cleaned reviews
